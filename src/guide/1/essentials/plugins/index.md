...
Kuzzle's **[Plugin Engine]({{ site_base_path }}plugins-reference/1)** is a powerful feature that ensures that Kuzzle meets any project requirement:
...
* [create your own plugin]({{ site_base_path }}plugins-reference/1/plugins-features) to meet your specific requirements.
...
[Hooks]({{ site_base_path }}plugins-reference/1/plugins-features/adding-hooks): add an asynchronous listener that performs operations triggered by data events. When the event occurs, the data is sent to the listener and Kuzzle continues its process without waiting for the listener to complete.
...
[Controllers & Routes]({{ site_base_path }}plugins-reference/1/plugins-features/adding-controllers): add a route to expose new entries in the API.

  _Example - "Expose a `checkout` API endpoint that handles a third-party payment process"_.

[Strategies]({{ site_base_path }}plugins-reference/1/plugins-features/adding-authentication-strategy): add an authentication strategy to identify and authenticate users.
...
[Protocols]({{ site_base_path }}plugins-reference/1/plugins-reference/protocols) add extended networking capabilities to your Kuzzle installation. These are useful if you need to handle various transport protocols.
...
To learn more about how to manage or configure plugins, please check our [Plugin Reference Documentation]({{ site_base_path }}plugins-reference/1/managing-plugins).

---

## Going Further

To get more insight into how plugins work, please refer to the [Plugin Reference]({{ site_base_path }}plugins-reference/1).
