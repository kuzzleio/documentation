...
| ``users`` | string | Filter [user notifications]({{ site_base_path }}sdk-reference/android/3/notifications/#user-notification) triggered upon a user entering the room (user: ``in``), leaving the room (user: ``out``), or both (user: ``all``). Setting this variable to ``none`` prevents receiving these notifications | ``none`` |
