...
Go to https://dl.kuzzle.io/sdk/csharp/1-dev and download the archive corresponding to your target artchitecture.

For amd64 linux :

``` bash
$ curl https://dl.kuzzle.io/sdk/csharp/1-dev/kuzzlesdk-csharp-experimental-amd64.tar.gz
```

Extract the archive :

``` bash
$ tar xf kuzzlesdk-csharp-experimental-amd64.tar.gz
```
...
``` sh
$ export KUZZLE_SDK_PATH=$PWD/kuzzle-csharp-sdk
```

## First connection

Create a `my-first-kuzzle-app.csharp` file and with the following source code :
...
``` bash
$ gcc -o my-first-kuzzle-app my-first-kuzzle-app.csharp -std=c++11 -lstdc++ -I${KUZZLE_SDK_PATH}/include -L${KUZZLE_SDK_PATH}/lib -lkuzzlesdk -lpthread -Wl,-rpath=${KUZZLE_SDK_PATH}/lib
$ ./my-first-kuzzle-app
...
Update `my-first-kuzzle-app.csharp` with the following content :
...
Create a `create-document.csharp` file with the following source code :
...
``` bash
$ gcc -o create-document create-document.csharp -std=c++11 -lstdc++ -I${KUZZLE_SDK_PATH}/include -L${KUZZLE_SDK_PATH}/lib -lkuzzlesdk -lpthread -Wl,-rpath=${KUZZLE_SDK_PATH}/lib
$ ./create-document
Connected to Kuzzle Server
